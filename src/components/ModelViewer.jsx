...
const ModelInner = ({
  ...
  /* scratch Vector3 pré-alloué : évite new THREE.Vector3() chaque frame (GC) */
  ...
  useFrame((_, dt) => {
    ...
    /* position via NDC — scratch réutilisé, zéro allocation GC */
    ndcScratch.current.copy(pivotW.current).project(camera);
    ndcScratch.current.x += xOff + cPar.current.x;
    ndcScratch.current.y += yOff + cPar.current.y;
    outer.current.position.copy(ndcScratch.current.unproject(camera));

    /* hover rotation delta */
    outer.current.rotation.x += cHov.current.x - phx;
    outer.current.rotation.y += cHov.current.y - phy;

    /* auto-rotation dt-based (frame-rate independent) */
  ...
  return (
    ...
    >
      ...
      <Canvas
        shadows
        frameloop={frameloop}
        gl={{ preserveDrawingBuffer: true, alpha: transparent, antialias: true }}
      ...
      >
        ...
        {showContactShadow && (
          <ContactShadows ref={contactRef} position={[0, -0.5, 0]} opacity={transparent ? 0.15 : 0.35} scale={10} blur={2} />
        )}
